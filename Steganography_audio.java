...
public class Steganography_audio {
   ...
   public static void file_processor(String[] args) {	
		...
		try {
			...
			if (mode.equals("##conceal")) {
				...
				if (sizeChecking(payload_size, wav_size)) {
					...
					*/
					...
					for (int i = 0; i < 4; i++) {
						...
						outStr.write(encode(payloadSize[i], encodeArray, littleEndian));
					}
					/*for (int i = 0; i < 4; i++) {
					...
					while (payload_size > 0 || (payloadBufferIndex < payloadBufferMax)) {
						...
						while (wavBufferIndex < wavBufferMax && payloadBufferIndex < payloadBufferMax) {
							System.arraycopy(wavBuffer, wavBufferIndex, encodeArray, 0, 16);
							encode(payloadBuffer[payloadBufferIndex], encodeArray, littleEndian);
							System.arraycopy(encodeArray, 0, processedBuffer, wavBufferIndex, 16);
							wavBufferIndex += 16;
							payloadBufferIndex++;
						}
					...
					fileWriter_meta.write(output.getName());
					...
					resultMsg = "File content is very large and fit into the cover object";
				}
				...
				for(int i = 0; i < 4; i++){
					...
					messageSizeBytes[i] = decode(wavBuffer, littleEndian);					
				}
				...
				if(sizeChecking(messageSize, wavDataSize)) {
					...
					for(long j = 0; j < messageSize; j++){
						...
						// retrieve the messageByte by calling decode
						messageByte = decode(wavBuffer, littleEndian);
			...
			e.printStackTrace();
			resultMsg = "File is not Successfully embeded!";
		}
	...
	//work with little-endian, unsigned numbers in Java
